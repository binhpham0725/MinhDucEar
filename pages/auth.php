...
<head>
  ...
  <!-- Google Identity Services (Sign-In & Sync) -->
...
<body class="flex flex-col min-h-screen selection:bg-primary selection:text-on-primary">
  ...
  <script>
        ...
        try {
          google.accounts.id.initialize({
            ...
            callback: async (response) => {
              ...
              showAlert('Đang xác thực Google Token & đồng bộ...', true);
              try {
                ...
                if (result.success) {
                  ...
                  setTimeout(() => { window.location.href = accountUrl; }, 800);
                } else {
                  showAlert(result.message || 'Đăng nhập Google thất bại!');
                }
              } catch (e) {
                showAlert('Lỗi kết nối máy chủ!');
              }
    ...
    function tryGoogleLogin() {
      showAlert('Đang kết nối với Google OAuth & đồng bộ YouTube Music...', true);
      setTimeout(async () => {
        try {
          const res = await fetch('api/endpoints/auth.php?action=google_login', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              google_id: 'google_user_demo_101',
              email: 'minhduc.audiophile@gmail.com',
              name: 'Minh Đức (Google)',
          ...
          if (result.success) {
            ...
            showAlert(result.error || 'Đăng nhập Google thất bại!');
          }
        } catch (err) {
