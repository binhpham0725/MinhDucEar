...
<body class="flex flex-col min-h-screen selection:bg-primary selection:text-on-primary">
  ...
  <main class="flex-1 flex items-center justify-center p-4 sm:p-6">
    <div class="w-full max-w-md bg-[#121217] rounded-2xl pixel-border p-6 sm:p-8 shadow-[0_0_60px_rgba(167,139,250,0.25)] relative">
      ...
      <div class="p-4 bg-[#181822] border border-secondary/40 rounded-xl mb-5 text-center shadow-inner">
        ...
        <div id="google-auth-button-slot" class="flex justify-center my-1.5">
          <button type="button" id="btn-google-instant-sync" class="w-full py-2.5 px-4 bg-white hover:bg-gray-100 text-gray-800 font-bold text-xs rounded-lg shadow-md flex items-center justify-center gap-3 transition-all hover:scale-[1.01] cursor-pointer">
  ...
  <script>
    ...
    function initGoogleAuth() {
      const slot = document.getElementById('google-auth-button-slot');
      if (!slot) return;
        ...
        try {
          window.google.accounts.id.initialize({
            ...
            callback: async (response) => {
              showAlert('Đang xác thực Google & đồng bộ...', true);
              try {
                let googleUser = null;
                ...
                const apiUrl = 'api/endpoints/auth.php?action=google_login';
                const res = await fetch(apiUrl, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ credential: response.credential, ...(googleUser || {}) })
                });
                const result = await res.json();
                const userToSave = (result && result.success && result.user) ? result.user : googleUser;
                const statsToSave = (result && result.sync_stats) ? result.sync_stats : { favorites_count: 0, playlists_count: 3, synced_tracks_count: 18, favorites: [] };
                if (userToSave) {
                  localStorage.setItem('minhduc_current_user', JSON.stringify(userToSave));
                  localStorage.setItem('minhduc_sync_stats', JSON.stringify(statsToSave));
                  showAlert('Đăng nhập Google thành công! Thư viện nhạc YouTube đã đồng bộ.', true);
                  setTimeout(() => { window.location.href = accountUrl; }, 600);
                  return;
                }
              } catch (e) {
    ...
    // Google Sign-In with instant sync button fallback
    const btnGoogleSync = document.getElementById('btn-google-instant-sync');
    if (btnGoogleSync) {
    ...
    async function tryGoogleLogin() {
      showAlert('Đang kết nối với Google...', true);
      try {
        ...
        console.warn('Firebase Google Auth:', authErr);
      }

      // Fallback: prompt for user's Google email if popup blocked
      const userEmail = prompt('Nhập địa chỉ email Google của bạn:');
      if (!userEmail || !userEmail.includes('@')) {
        showAlert('Vui lòng nhập địa chỉ email hợp lệ!');
        return;
      }
      const userName = prompt('Nhập tên hiển thị (tùy chọn):') || userEmail.split('@')[0];
      try {
        const res = await fetch('api/endpoints/auth.php?action=google_login', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        const result = await res.json();
        if (result.success) {
          showAlert('Đăng nhập Google thành công! Đang chuyển hướng...', true);
          setTimeout(() => { window.location.href = accountUrl; }, 600);
        } else {
